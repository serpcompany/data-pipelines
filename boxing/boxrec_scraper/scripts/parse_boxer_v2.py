...
def parse_date_of_birth(soup):
    ...
    # Look for birth date in different formats
    patterns = [
        r'born:\s*(\d{4}-\d{2}-\d{2})',
        r'birth date:\s*(\d{4}-\d{2}-\d{2})',
        r'(\d{4}-\d{2}-\d{2})\s*\(age',
    ]
...
def extract_event_bout_links(row):
    """Extract event and bout page links from a fight row."""
    event_link = None
    bout_link = None
...
def parse_boxer_html(html_path):
    ...
    # Initialize data with v2 schema fields
    data = {
        'boxrec_id': None,
        'boxrec_url': None,
        'boxrec_wiki_url': None,
        'slug': None,
        'full_name': None,
        'birth_name': None,
        'nickname': None,
        ...
        'pro_division': None,
        'pro_wins': None,
        'pro_wins_by_knockout': None,
        'pro_losses': None,
        'pro_losses_by_knockout': None,
        'pro_draws': None,
        'pro_status': None,
        'pro_total_bouts': None,
        'pro_total_rounds': None,
    ...
    if title_tag:
        ...
        if 'BoxRec:' in title_text:
            data['full_name'] = title_text.replace('BoxRec:', '').strip()
            # Generate slug from name
            if data['full_name']:
                data['slug'] = re.sub(r'[^a-z0-9]+', '-', data['full_name'].lower()).strip('-')
    
    # Extract profile image
    data['image_url'] = extract_profile_image(soup)
    
    # Extract date of birth
    data['date_of_birth'] = parse_date_of_birth(soup)
            ...
            for row in rows:
                ...
                if len(cells) >= 2:
                    ...
                    if 'birth name' in label:
                        ...
                        data['stance'] = value.lower() if value else None
                    elif 'height' in label:
                        data['height'] = value
                    elif 'reach' in label:
                        data['reach'] = value
                    elif 'weight' in label and 'division' not in label:
                        data['weight'] = value
                        ...
                        data['pro_total_bouts'] = int(value) if value.isdigit() else None
                    elif 'rounds' in label:
                        data['pro_total_rounds'] = int(value) if value.isdigit() else None
                    elif label == 'kos':
                        ...
                        if status_value:
                            data['pro_status'] = 'active' if 'active' in status_value else 'inactive'
                        ...
                        data['pro_debut_date'] = value
                    elif 'sex' in label or 'gender' in label:
                        gender_value = value.lower() if value else None
                        if gender_value:
                            data['gender'] = 'male' if 'male' in gender_value else 'female' if 'female' in gender_value else None
                    elif 'id#' in label:
    ...
    if wiki_link:
        data['boxrec_wiki_url'] = 'https://boxrec.com' + wiki_link['href']
        ...
        if wld_table:
            ...
            if first_row:
                ...
                if len(cells) >= 3:
                    ...
                    # Set record fields
                    data['pro_wins'] = int(wins) if wins and wins.isdigit() else 0
                    data['pro_losses'] = int(losses) if losses and losses.isdigit() else 0
                    data['pro_draws'] = int(draws) if draws and draws.isdigit() else 0
                    data['pro_wins_by_knockout'] = int(kos) if kos else 0
    ...
    if data_table:
        ...
        for row in rows:
            ...
            if len(cells) >= 6:
                ...
                event_link, bout_link = extract_event_bout_links(row)
                
                bout = {
                    'bout_date': clean_text(date_cell.get_text()),
                    'opponent_name': opponent_name,
                    'opponent_id': opponent_id,
                    'opponent_url': opponent_url,
                    'opponent_weight': None,  # Not available in this view
                    'opponent_record': opponent_record,
                    'recent_form': recent_form,
                    'venue_name': venue,
                    'referee_name': None,  # Not available in this view
                    'judge_1_name': None,  # Not available in this view
                    'judge_1_score': None,
                    'judge_2_name': None,
                    'judge_2_score': None,
                    'judge_3_name': None,
                    'judge_3_score': None,
                    'num_rounds_scheduled': None,  # Not available in this view
                    'result': result,
                    'result_method': result_method,
                    'result_round': result_round,
                    'event_page_link': event_link,
                    'bout_page_link': bout_link,
                    'scorecards_page_link': None,  # Not available in this view
                    'title_fight': None,  # Not available in this view
                    'bout_rating': bout_rating
                }
                
                # Only add if we have meaningful data
                if bout['bout_date'] and bout['opponent_name']:

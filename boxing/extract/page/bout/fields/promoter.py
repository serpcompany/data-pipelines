...
def extract(soup: BeautifulSoup) -> Optional[str]:
    """
    Extract promoter from bout page HTML.

    Args:
        soup: BeautifulSoup object of the bout page

    Returns:
        str: promoter data or None if not found
    """
    role_row = None
    for tr in soup.find_all("tr"):
        b = tr.find("b")
        if b and b.get_text(strip=True).lower() == "promoter":
            role_row = tr
            break
            ...
            if not next_td:
                ...
                if inner_tr:
                    ...
                    for i, td in enumerate(tds):
                        if td.find(
                            "b", string=lambda s: s and s.strip().lower() == "promoter"
                        ):
                            if i + 1 < len(tds):
                                next_td = tds[i + 1]
                            break

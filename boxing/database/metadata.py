...
class MetadataTracker:
    ...
    def track_scraped_html(
        self, boxrec_url: str, boxrec_id: str, html_content: str, scraped_at: datetime
    ) -> Dict:
        ...
        if existing:
            ...
            logger.info(
                f"Updated metadata for {boxrec_url}, change detected: {change_detected}"
            )
    ...
    def get_stats(self) -> Dict:
        ...
        # Extracted URLs
        cursor.execute(
            "SELECT COUNT(*) FROM data_lake_metadata WHERE extracted_at IS NOT NULL"
        )
        stats["extracted_urls"] = cursor.fetchone()[0]

        # URLs with changes
        cursor.execute(
            "SELECT COUNT(*) FROM data_lake_metadata WHERE change_detected = TRUE"
        )

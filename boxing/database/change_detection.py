    ...
    def bulk_check_changes(self, limit: int = 100) -> Dict:
        ...
        for url_info in urls_to_check:
            try:
                ...
                if not result:
                    logger.warning(f"No HTML found in data lake for {url_info['boxrec_url']}")
                    continue
                ...
                change_result = self.check_for_changes(
                    boxrec_url=url_info['boxrec_url'],
        ...
        logger.info(f"Change detection complete: {checked} checked, {changed} changed, {errors} errors")
    ...
    def get_change_report(self) -> Dict:
        ...
        with MetadataTracker() as tracker:
            ...
            for change in changed_urls:
                date_key = change['last_checked_at'].date() if change['last_checked_at'] else 'unknown'
            ...
            report['recent_changes'] = [
                change for change in changed_urls 
                if change['last_checked_at'] and change['last_checked_at'] > recent_cutoff
            ]
